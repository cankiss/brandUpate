/*
 * @Author: yucccc
 * @Date: 2017-11-20 14:52:20
 * @Please see it : https://github.com/yucccc/vue-pay-keyboard
 */
<template>
    <div class="pay-box" v-show="isPay">
        <transition name="slide">
            <div v-if="isPay">
                <!-- 标题 -->
                <div class="title v-1px-b pr">
                    <span class="font-size-18">{{payTitle}}</span>
                ...
                <!--解释的废话，输入交易密码还废话-->
                <div class="explain" v-show="keyShow">
                    <slot></slot>
                ...
                <div class="explain" v-show="keyShow">
                    <span class="font-size-12 font-666">请输入平台6位数字交易密码</span>
                    <router-link class="font-size-12" style="color:#FF665E" to='/mine/setup/payPwdSet/1'>忘记交易密码?</router-link>
                </div>
                <!--keyboard-->
                <div class="key-box" v-if="keyShow">
                    ...
                    <div class="item v-1px-t" v-for="(item, i) in keyList" :key="i">
                        <div class="key" v-for="(val, key) in item" :key="key" @click="inputStart(val, $event)" @touchend="inputEnd($event)" :class="{'v-1px-l':key!=0}">
                            {{val}}
                        </div>
                    </div>
                    <div class="item v-1px-t">
                        <div class="key" style="background: #D1D5DB"></div>
                        <div class="key v-1px-l" @touchstart="inputStart(0, $event)" @touchend="inputEnd($event)">0
                        </div>
                        <div class="key v-1px-l" style="background: #D1D5DB" @touchstart="del($event)" @touchend="inputEnd($event,'del')">
                            <span class="del-style"><i>×</i></span>
                        </div>
                    </div>
                </div>
                <!-- 密码输入完毕动画 -->
    ...
    export default {
        ...
        methods: {
            ...
            inputStart(val, e) {
                ...
                let eCurrent = e.currentTarget;
                setTimeout(()=>{
                	this.unhighLight(eCurrent);
                },200);
            },
            // 删除输入
            del() {
