    ...
    <div class="pay-box" v-show="isPay">
        <transition name="slide">
            <div v-if="isPay">
                ...
                <!--输入的密码-->
                <div class="contentdown-container" v-show="keyShow">
                    <div class="pas-box v-1px">
                        <div v-for="(pas,i) in pasDigits" :key="i">
                            <input type="password" :value="val[i]" disabled>
                        </div>
                    </div>
                    <div class="count-down font-999">
                        <countdown v-model="time" @on-finish="finish" v-show="false" :start='show'></countdown>
                        <p v-show="show">{{`${time}s`}}</p>
                        <button @click="refetch" v-show="!show">重新获取</button>
                    </div>
                </div>
                <div class="explain" v-show="keyShow">
                    <span class="font-size-12 font-999">短信已发送至手机号 {{phoneNum|phoneHidden}}</span>
                </div>
                <!--keyboard-->
